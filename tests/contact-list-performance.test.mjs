...
test("contacts page uses server pagination and database-side filtering", async () => {
  ...
  assert.match(view, /Контакты по ответственным/);
  ...
  assert.match(store, /includeMeta = url\.searchParams\.get\("meta"\) !== "0"/);
  assert.match(store, /readyTelegram/);
...
test("large contact payloads are loaded only by workflows that require them", async () => {
  ...
  assert.match(wizard, /\/api\/contacts\?page=1&pageSize=250&meta=0&delivery=pending/);
