...
test("manual controls export a single-page A4 PDF with working links", async () => {
  ...
  assert.match(exports, /Здравствуйте,\\\\s\*/);
  assert.match(exports, /добрый день\\\\\./);
  assert.match(exports, /pdfEditableGreeting/);
  assert.match(exports, /createTextField\(index === 0 \? "recipient_greeting"/);
  ...
  assert.match(exports, /PDFName\.of\("DA"\), PDFString\.of\(defaultAppearance\)/);
  assert.match(exports, /PDFName\.of\("NeedAppearances"\), PDFBool\.False/);
  assert.match(exports, /PDFName\.of\("DR"\)/);
  assert.match(exports, /formFonts\.set\(PDFName\.of\(fieldFont\.name\), fieldFont\.ref\)/);
  assert.doesNotMatch(exports, /StandardFonts\.(?:TimesRoman|Helvetica)/);
  assert.match(exports, /defaultAppearance = `\$\{colorOperator\} \/\$\{fieldFont\.name\}/);
  ...
  assert.match(exports, /Нажмите на строку приветствия и замените слово «Имя»/);
...
test("decor library retains thirty technology motifs, adds general patterns, and forty email frames", async () => {
  ...
  assert.equal((patternNames.match(/"[^"]+"/g) ?? []).length, 30);
  assert.equal((frames.match(/\{ id: "/g) ?? []).length, 40);
  ...
  assert.match(library, /102 фона и орнамента/);
