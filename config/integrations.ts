...
export const deliveryChannels: DeliveryChannelDefinition[] = [
  ...
  },
  {
    ...
    contactField: "Подписчики подключённого бота",
...
export const integrationProviders: IntegrationProviderDefinition[] = [
  ...
  },
  {
    ...
    summary:
      "Рассылки через вашего бота подписчикам, которые подтвердили получение сообщений.",
    channelIds: ["telegram"],
    accent: "#229ed9",
    initials: "TG",
    ...
    setupSteps: [
      "Создать бота в BotFather и вставить его токен в Потоке.",
      "Поделиться ссылкой: после подтверждения подписки контакт добавится автоматически.",
      "Выбрать подписчиков, написать сообщение и запустить кампанию сейчас или по расписанию.",
    ],
    limitations: [
