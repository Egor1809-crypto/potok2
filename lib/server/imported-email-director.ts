...
action=rebuild: исходник — письмо-картинка (PNG/JPG или страницы PDF/Word). Прочитай ВСЕ переданные страницы, воссоздай письмо как редактируемый HTML с живым текстом, inline CSS и таблицами, адаптивной шириной 620–680px. Соблюди userCommand. Сохрани ВСЁ читаемое содержание, порядок, факты, имена, даты, логотип, ключевой визуальный образ и характер оформления. Не придумывай недостающие данные. Неразборчивые фрагменты отметь в notes. Не вставляй целый исходный скриншот вместо вёрстки и не дублируй текст картинкой. Для фотографий/логотипов/иллюстраций используй crops: source — точное поле source из attachedImages; x,y,width,height — доли ширины/высоты исходника от 0 до 1. Вырезай только нужную иллюстрацию, без окружающего текста. В HTML используй src="{{crop:ID}}", например {{crop:hero}}. Не вырезай надписи вместо живого текста, кроме логотипа. До 6 непустых crops. Не придумывай изображения и URL. Нельзя узнать адрес кнопки по её пикселям: если ссылки нет в sourceHtml или userCommand, отрисуй её без href и укажи в notes, какой адрес нужно добавить. Не используй фиктивные #, example.com или пустые ссылки. patches=[].
...
export async function directImportedEmail(request: Request, value: unknown): Promise<ImportDirection> {
  ...
  const call = async (model: string, correction = "") => {
    ...
    const response = await fetch(provider.endpoint, { method: "POST", headers: { Authorization: `Bearer ${provider.key}`, "Content-Type": "application/json" }, body: JSON.stringify(payload), signal: AbortSignal.timeout(150_000) });
