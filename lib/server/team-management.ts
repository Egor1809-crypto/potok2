...
async function validateBases(scope: ContactAccessScope) {
  ...
  const rows = await getD1().prepare("SELECT id FROM participants WHERE workspace_id=? AND id IN (SELECT value FROM json_each(?))").bind(WORKSPACE_ID, JSON.stringify(scope.baseIds)).all();
...
function audit(actorId: string, targetId: string, action: string, details: Record<string, unknown>) {
  return getD1().prepare("INSERT INTO team_access_events (id,workspace_id,actor_id,target_id,action,details,created_at) VALUES (?,?,?,?,?,?,?)").bind(newId("team-event"),WORKSPACE_ID,actorId,targetId,action,JSON.stringify(details),new Date().toISOString());
}
export async function teamOverview(request: Request) {
  ...
  if (!canManage) {
    const assigned = await getD1().prepare("SELECT id,display_name AS label,0 AS count FROM participants WHERE workspace_id=? AND id IN (SELECT value FROM json_each(?))").bind(WORKSPACE_ID, JSON.stringify(participant.accessScope.baseIds)).all();
  ...
  const [bases, groups, invites, events] = await Promise.all([
    db.prepare("SELECT coalesce(responsible_participant_id,created_by_participant_id) AS id,count(*) AS count FROM contacts WHERE workspace_id=? GROUP BY coalesce(responsible_participant_id,created_by_participant_id)").bind(WORKSPACE_ID).all<{id:string;count:number}>(),
    db.prepare("SELECT j.value AS label,count(*) AS count FROM contacts,json_each(contacts.tags) j WHERE contacts.workspace_id=? AND j.type='text' GROUP BY j.value ORDER BY j.value LIMIT 2000").bind(WORKSPACE_ID).all(),
    db.prepare("SELECT id,label,role,access_scope,target_participant_id,expires_at,use_count,revoked_at,accepted_participant_id,created_at FROM team_invites WHERE workspace_id=? ORDER BY created_at DESC LIMIT 50").bind(WORKSPACE_ID).all(),
    db.prepare("SELECT e.*,p.display_name AS actor_name FROM team_access_events e LEFT JOIN participants p ON p.id=e.actor_id WHERE e.workspace_id=? ORDER BY e.created_at DESC LIMIT 30").bind(WORKSPACE_ID).all(),
  ]);
...
export async function manageTeam(request: Request, payload: unknown) {
  ...
  if (action === "revoke_invite") {
    ...
    await db.batch([db.prepare("UPDATE team_invites SET revoked_at=? WHERE id=? AND workspace_id=? AND use_count=0").bind(now,id,WORKSPACE_ID),audit(actor.id,id,"invite_revoked",{})]);
  ...
  if (action === "invite") {
    ...
    if (targetId) { const target=await accessParticipant(targetId); if (target.workspaceId !== WORKSPACE_ID || target.login) throw new ApiRequestError("У участника уже есть аккаунт. Измените его доступ в списке команды.",409); }
    ...
    await db.batch([db.prepare("INSERT INTO team_invites (id,workspace_id,code_hash,created_by_participant_id,expires_at,max_uses,use_count,role,access_scope,label,target_participant_id,created_at) VALUES (?,?,?,?,?,1,0,?,?,?,?,?)").bind(id,WORKSPACE_ID,await sha256(code),actor.id,expiresAt,access.role,JSON.stringify(access.accessScope),label,targetId,now),audit(actor.id,id,"invite_created",{label,...access})]);
  ...
  if (previous.workspaceId!==WORKSPACE_ID) throw new ApiRequestError("Участник не найден.",404);
  ...
  if (previous.role === "admin" && (access.role !== "admin" || status !== "active")) {
    const other = await db.prepare("SELECT id FROM participants WHERE workspace_id=? AND id<>? AND role='admin' AND status='active' AND password_hash IS NOT NULL LIMIT 1").bind(WORKSPACE_ID,id).first();
  ...
  if (reduced) {
    const scheduled=await db.prepare("SELECT id FROM campaigns WHERE workspace_id=? AND participant_id=? AND status='scheduled'").bind(WORKSPACE_ID,id).all<{id:string}>();
  ...
  const update=db.prepare(`UPDATE participants SET role=?,access_scope=?,status=?,updated_at=? WHERE id=? AND workspace_id=? AND updated_at=? AND ${lastAdminGuard} AND EXISTS (SELECT 1 FROM participants executor WHERE executor.id=? AND executor.role='admin' AND executor.status='active')`).bind(access.role,JSON.stringify(access.accessScope),status,now,id,WORKSPACE_ID,expected,access.role,status,actor.id);
  const statements=[update];
  if (access.role !== "admin" || status === "disabled") statements.push(db.prepare("UPDATE team_invites SET revoked_at=? WHERE workspace_id=? AND created_by_participant_id=? AND use_count=0 AND revoked_at IS NULL AND EXISTS (SELECT 1 FROM participants WHERE id=? AND updated_at=?)").bind(now,WORKSPACE_ID,id,id,now));
  ...
  statements.push(db.prepare("INSERT INTO team_access_events (id,workspace_id,actor_id,target_id,action,details,created_at) SELECT ?,?,?,?,'member_updated',?,? WHERE EXISTS (SELECT 1 FROM participants WHERE id=? AND updated_at=?)").bind(newId("team-event"),WORKSPACE_ID,actor.id,id,JSON.stringify({before:{role:previous.role,accessScope:previous.accessScope,status:previous.status},after:{...access,status}}),now,id,now));
