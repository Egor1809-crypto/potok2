...
async function seedDatabase(request: Request) {
  ...
  const [conferenceBriefTemplateState] = await db
    .select({ key: systemState.key })
    .from(systemState)
    .where(eq(systemState.key, "email-template-library-v20-conference-brief-clean-subject"))
    .limit(1);
  if (!conferenceBriefTemplateState) {
    ...
    await db.insert(systemState).values({
      key: "email-template-library-v20-conference-brief-clean-subject",
      value: "seeded",
      updatedAt: now,
    }).onConflictDoUpdate({
      target: systemState.key,
  ...
  const [conferencePdfEditableGreetingState] = await db
    .select({ key: systemState.key })
    .from(systemState)
    .where(eq(systemState.key, "conference-production-html-v18-editable-greeting-logo"))
    .limit(1);
  if (!conferencePdfEditableGreetingState) {
    for (const template of conferenceProductionTemplateValues.filter(
      (item) => !item.id.endsWith("-11-cost-reduction"),
    )) {
      await db
        .update(emailTemplates)
        .set({
    ...
    await db.insert(systemState).values({
      key: "conference-production-html-v18-editable-greeting-logo",
      value: "updated",
      updatedAt: now,
    }).onConflictDoUpdate({
      target: systemState.key,
  ...
  const [conferencePersonalInvitationManyashaState] = await db
    .select({ key: systemState.key })
    .from(systemState)
    .where(eq(systemState.key, "conference-production-html-v19-personal-invitation-manyasha"))
    .limit(1);
  if (!conferencePersonalInvitationManyashaState) {
    const template = conferenceProductionTemplateValues.find(
      (item) => item.id === "template-user-conference-01-personal-invitation",
    );
    if (template) {
    ...
    await db.insert(systemState).values({
      key: "conference-production-html-v19-personal-invitation-manyasha",
      value: "updated",
      updatedAt: now,
    }).onConflictDoUpdate({
      target: systemState.key,
