...
PDF — цельное изображение: текст и рисунки внутри него нельзя менять патчами. Если задача требует этого, объясни ограничение и оставь patches=[]. Каждый patch содержит РОВНО три поля: {"target":"id элемента","field":"fontSize","value":"32"}. Для изменения цвета нужен отдельный patch с field="color". Не объединяй несколько полей элемента в одном patch. Неизменённые значения не присылай. Не заявляй, что исправил то, для чего нет patches.`;
  ...
  const reviewModel = body.action === "review" && provider.provider === "navyai" &&
    provider.visionModel === "gemini-3.8-flash" && /^gpt-5(?:[.-]|$)/.test(provider.model)
      ? provider.model : provider.visionModel;
  ...
  const callModel = async (model: string, signal: AbortSignal) => {
    ...
    try { data = asObject(await response.json()); }
    catch {
      signal.throwIfAborted();
      throw new ApiRequestError("ИИ вернул неполный ответ. Повторите проверку слайда.", 502);
    }
    if (!response.ok)
      throw new ApiRequestError(
        response.status === 429
        ...
        502, // Provider congestion is per-slide; only our hourly quota stops the deck.
      );
    try {
      const text =
        provider.provider === "navyai"
  ...
  if (body.action === "review") {
    // Both attempts retain the same screenshot, context and strict result validation.
    // Use the configured GPT model only when it is a known vision-capable family.
  ...
  try { return await callModel(model, signal); }
    ...
    if (!request.signal.aborted && signal.aborted)
      throw new ApiRequestError("ИИ не завершил запрос вовремя. Готовые разборы сохранены.", 504);
    throw error;
  }
}
