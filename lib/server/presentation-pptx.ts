...
function presentationPatternShapes(
  ...
  const soft = dark(project.backgroundColor)
    ? project.themeId === "noir"
      ? "#263746"
      : "#292330"
    : project.themeId === "ocean"
      ? "#D9F1EE"
    ...
    for (let row = 0; row < 3; row += 1)
      for (let column = 0; column < 4; column += 1) {
        shapes.push(
          rect(
            id++,
            `Узор · точка ${row + 1}.${column + 1}`,
            (10.95 + column * 0.22) * EMU,
            (0.35 + row * 0.22) * EMU,
            0.055 * EMU,
            0.055 * EMU,
            project.accentColor,
  ...
  } else if (project.themeId === "premium") {
    shapes.push(
      rect(
        id++,
        "Премиум · верхняя линия",
        0.65 * EMU,
        0.45 * EMU,
        11.0 * EMU,
        0.035 * EMU,
...
function slideShapes(
  ...
  if (slide.layout === "title") {
    ...
    if (slide.body)
      shapes.push(
        textBox(
          ...
          contentWidth * 0.9,
          1.25 * EMU,
          [{ text: slide.body, options: { color: muted, fontSize: 23 } }],
        ),
      );
    shapes.push(
      textBox(id++, "Маркер", x, 6.62 * EMU, 2.2 * EMU, 0.3 * EMU, [
        {
          text: "ПРЕЗЕНТАЦИИ «ПОТОК»",
    ...
    shapes.push(
      textBox(id++, "Главная мысль", x, 1.35 * EMU, contentWidth, 3.2 * EMU, [
        {
          text: slide.title,
          options: {
            color: text,
    ...
    if (slide.body)
      shapes.push(
        textBox(id++, "Пояснение", x, 4.9 * EMU, contentWidth, 1.1 * EMU, [
          { text: slide.body, options: { color: muted, fontSize: 19 } },
        ]),
      );
    ...
    if (slide.body)
      shapes.push(
        textBox(id++, "Текст", x, 3.2 * EMU, leftWidth, 2.55 * EMU, [
      ...
      shapes.push(
        textBox(
          ...
          items.slice(0, 5).map((item) => ({
            text: item,
            options: { color: text, fontSize: 19, bullet: true, margin: 0 },
          })),
        ),
      );
    }
    ...
    shapes.push(
      textBox(id++, "Цитата", x, 1.25 * EMU, contentWidth, 3.7 * EMU, [
        {
          text: `“${slide.title.replace(/^“|”$/g, "")}”`,
          options: {
            color: text,
            fontSize: image ? 32 : 41,
            bold: true,
            italic: true,
            fontFace: "Georgia",
            valign: "ctr",
    ...
    items.forEach((item, index) => {
      const [metric = "—", label = "показатель"] = item
        .split("|")
        .map((part) => part.trim());
      ...
      shapes.push(
        textBox(
          ...
          [
            {
              text: metric,
              options: { color: accent, fontSize: 40, bold: true },
            },
    ...
    if (slide.body)
      shapes.push(
        textBox(id++, "Контекст", x, 5.55 * EMU, contentWidth, 0.65 * EMU, [
          { text: slide.body, options: { color: muted, fontSize: 14 } },
        ]),
      );
      ...
      shapes.push(
        textBox(
          ...
          [
            {
              text: slide.body,
              options: { color: inverse, fontSize: 19, align: "ctr" },
            },
          ],
        ),
      );
    ...
    const items = slide.bullets.length
      ? slide.bullets
      : [
          "Добавьте первый аргумент",
          "Добавьте второй аргумент",
          "Сформулируйте вывод",
        ];
    shapes.push(
      textBox(
  ...
  if (slide.ctaUrl) {
    shapes.push(
      textBox(
        ...
        [
          {
            text: slide.ctaLabel || "Узнать подробнее",
            options: {
              color: slide.layout === "closing" ? accent : inverse,
              fontSize: 13,
              bold: true,
              align: "ctr",
              valign: "ctr",
              margin: Math.round(0.08 * EMU),
            },
          },
        ],
        {
          fill: slide.layout === "closing" ? "#FFFFFF" : accent,
          radius: true,
          hyperlinkId: "rId50",
        },
      ),
  ...
  if (slide.socialLinks?.length) {
    slide.socialLinks.slice(0, 5).forEach((link, index) => {
      shapes.push(
        textBox(
          id++,
          `Социальная ссылка ${index + 1}`,
          (4.25 + index * 1.65) * EMU,
          6.05 * EMU,
          1.5 * EMU,
          0.35 * EMU,
          [
            {
              text: link.label,
              options: {
                color: slide.layout === "closing" ? inverse : accent,
                fontSize: 10,
                bold: true,
                align: "ctr",
              },
...
function contentTypes(slides: number, images: ImageEntry[]) {
  const imageDefaults = [...new Set(images.map((image) => image.extension))]
    .map(
      (extension) =>
        `<Default Extension="${extension}" ContentType="${extension === "jpg" ? "image/jpeg" : `image/${extension}`}"/>`,
    )
    .join("");
...
function packageEntries(
  ...
  const slideIds = project.slides
    ...
    .join("");
  const slideRels = project.slides
    .map(
      (_, index) =>
  ...
  const xmlFile = (name: string, content: string) =>
    files.push({ name, bytes: utf8(content) });
  xmlFile(
    "[Content_Types].xml",
    contentTypes(
  ...
  xmlFile(
    "docProps/app.xml",
    `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Properties xmlns="http://schemas.openxmlformats.org/officeDocument/2006/extended-properties" xmlns:vt="http://schemas.openxmlformats.org/officeDocument/2006/docPropsVTypes"><Application>Поток</Application><PresentationFormat>On-screen Show (16:9)</PresentationFormat><Slides>${project.slides.length}</Slides><Notes>0</Notes><HiddenSlides>0</HiddenSlides><MMClips>0</MMClips><ScaleCrop>false</ScaleCrop><Company>Поток</Company><AppVersion>1.0</AppVersion></Properties>`,
  );
  xmlFile(
  ...
  xmlFile(
    "ppt/viewProps.xml",
    '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><p:viewPr xmlns:a="http://schemas.openxmlformats.org/drawingml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" xmlns:p="http://schemas.openxmlformats.org/presentationml/2006/main"><p:normalViewPr><p:restoredLeft sz="15620"/><p:restoredTop sz="94660"/></p:normalViewPr><p:slideViewPr><p:cSldViewPr snapToGrid="1" snapToObjects="1"/></p:slideViewPr><p:notesTextViewPr><p:cViewPr varScale="1"><p:scale><a:sx n="100" d="100"/><a:sy n="100" d="100"/></p:scale><p:origin x="0" y="0"/></p:cViewPr></p:notesTextViewPr><p:gridSpacing cx="72008" cy="72008"/></p:viewPr>',
  );
  xmlFile(
    "ppt/tableStyles.xml",
  ...
  project.slides.forEach((slide, index) => {
    ...
    const hyperlinkRelationships = [
      ...
      ...(slide.socialLinks ?? [])
        .slice(0, 5)
        .map((link, linkIndex) => ({ id: 51 + linkIndex, url: link.url })),
    ]
      .map(
        (link) =>
          `<Relationship Id="rId${link.id}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/hyperlink" Target="${xml(link.url)}" TargetMode="External"/>`,
      )
      .join("");
    xmlFile(
    ...
    if (image) {
      imageIndex += 1;
      files.push({
        name: `ppt/media/image${imageIndex}.${image.extension}`,
        bytes: image.bytes,
      });
    }
  });
