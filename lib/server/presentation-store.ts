...
const SOURCES = new Set<PresentationSourceType>([
  "blank",
  "template",
  "ai",
  "email",
...
function color(value: unknown, field: string, fallback: string) {
  ...
  if (!HEX.test(result))
    throw new ApiRequestError(
      `Поле «${field}» должно содержать цвет в формате #RRGGBB.`,
    );
...
function parseBullets(value: unknown, index: number): string[] {
  ...
  if (!Array.isArray(value))
    throw new ApiRequestError(
      `Список на слайде ${index + 1} имеет неверный формат.`,
    );
  return value
    .slice(0, 8)
    .map((item) => cleanText(item, `Пункт слайда ${index + 1}`, 240))
    .filter(Boolean);
}
...
function parseSlide(value: unknown, index: number): PresentationSlide {
  ...
  if (!LAYOUTS.has(rawLayout as PresentationSlideLayout))
    throw new ApiRequestError(`Выберите допустимый макет слайда ${index + 1}.`);
  const assetId =
    optionalText(object.assetId, `Изображение слайда ${index + 1}`, 160) ??
    assetIdFromImageUrl(object.imageUrl);
  const safeLink = (value: unknown, label: string) => {
    const url = optionalText(value, label, 2_000);
  ...
  const socialLinks = Array.isArray(object.socialLinks)
    ? object.socialLinks.slice(0, 8).flatMap((value, socialIndex) => {
        if (!value || typeof value !== "object" || Array.isArray(value))
          return [];
        const row = value as Record<string, unknown>;
        const label = optionalText(
          row.label,
          `Название социальной сети ${socialIndex + 1}`,
          80,
        );
        const url = safeLink(
          row.url,
          `Ссылка социальной сети ${socialIndex + 1}`,
        );
  ...
  return {
    ...
    speakerNotes:
      optionalText(object.speakerNotes, `Заметки слайда ${index + 1}`, 3_000) ??
      "",
    ...(assetId ? { assetId } : {}),
    ...
    ...(optionalText(object.ctaLabel, `Текст кнопки слайда ${index + 1}`, 100)
      ? {
          ctaLabel: optionalText(
            object.ctaLabel,
            `Текст кнопки слайда ${index + 1}`,
            100,
          ),
        }
      : {}),
    ...(safeLink(object.ctaUrl, `Ссылка кнопки слайда ${index + 1}`)
...
function parseSlides(
  value: unknown,
  fallback = defaultPresentationSlides,
): PresentationSlide[] {
  if (value === undefined)
  ...
  if (!Array.isArray(value) || value.length < 1 || value.length > 40) {
    throw new ApiRequestError(
      "Презентация должна содержать от 1 до 40 слайдов.",
    );
  }
...
function sourceType(
  value: unknown,
  fallback: PresentationSourceType,
): PresentationSourceType {
  if (value === undefined) return fallback;
  const result = cleanText(
    ...
    20,
  ) as PresentationSourceType;
  if (!SOURCES.has(result))
    throw new ApiRequestError("Указан неизвестный источник презентации.");
  return result;
}

function themeId(
  value: unknown,
  fallback: PresentationThemeId,
): PresentationThemeId {
  if (value === undefined) return fallback;
  const result = cleanText(
    value,
    "Тема презентации",
    30,
  ) as PresentationThemeId;
  if (!THEMES.has(result))
    throw new ApiRequestError("Выберите допустимую тему презентации.");
  return result;
}

function toRecord(
  row: typeof presentationProjects.$inferSelect,
): PresentationProjectRecord {
  return {
    id: row.id,
...
function slidesFromEmailTemplate(
  row: typeof emailTemplates.$inferSelect,
): PresentationSlide[] {
  ...
  for (const block of row.builderDocument.blocks) {
    if (
      slides.length >= 10 ||
      ["logo", "divider", "spacer", "social", "footer", "pattern"].includes(
        block.type,
      )
    )
      continue;
    const parts = splitContent(block.content);
    const title = plainBlockText(
      parts[0] || block.label || "Содержание письма",
    );
    ...
    if (block.type === "image") {
      ...
      slides.push({
        ...common,
        layout: "split",
        title: block.label || "Визуальный материал",
        body: "",
        bullets: [],
      });
      ...
      slides.push({
        ...common,
        layout: "stats",
        bullets: parts.map(plainBlockText).filter(Boolean).slice(0, 6),
      });
    } else if (["checklist", "timeline", "faq"].includes(block.type)) {
      slides.push({
        ...common,
        layout: "bullets",
        bullets: parts.map(plainBlockText).filter(Boolean).slice(0, 8),
      });
    } else if (
      [
        "columns",
      ...
      slides.push({
        ...common,
        layout: "closing",
        body: block.href ? `Ссылка: ${block.href}` : common.body,
      });
    } else {
      slides.push({
        ...common,
...
> {
  const object = asObject(value);
  const sourceEmailTemplateId =
    object.sourceEmailTemplateId === null
      ? null
      : (optionalText(object.sourceEmailTemplateId, "Email-шаблон", 160) ??
        null);
  let emailSource: typeof emailTemplates.$inferSelect | undefined;
  if (sourceEmailTemplateId) {
    [emailSource] = await getDb()
      .select()
      .from(emailTemplates)
      .where(
        and(
      ...
      .limit(1);
    if (!emailSource)
      throw new ApiRequestError("Email-шаблон для презентации не найден.", 404);
  }
  ...
  return {
    name:
      cleanText(
        object.name ?? emailSource?.name ?? "Новая презентация",
        "Название презентации",
        120,
      ) || "Новая презентация",
    description:
      optionalText(object.description, "Описание презентации", 500) ??
      (emailSource ? `Создано из письма «${emailSource.name}».` : ""),
    ...
    slides:
      emailSource && object.slides === undefined
        ? slidesFromEmailTemplate(emailSource)
        : parseSlides(object.slides),
    sourceType: sourceType(object.sourceType, emailSource ? "email" : "blank"),
    sourceEmailTemplateId,
  };
}

export async function listPresentationProjects(
  request: Request,
...
export async function getPresentationProject(
  request: Request,
  idValue: unknown,
): Promise<PresentationMutationResponse> {
  await ensureDatabase(request);
...
export async function createPresentationProject(
  request: Request,
  value: unknown,
): Promise<PresentationMutationResponse> {
  await ensureDatabase(request);
...
export async function updatePresentationProject(
  request: Request,
  value: unknown,
): Promise<PresentationMutationResponse> {
  await ensureDatabase(request);
  ...
  const expectedUpdatedAt = optionalText(
  ...
  if (expectedUpdatedAt && expectedUpdatedAt !== current.updatedAt) {
    throw new ApiRequestError(
      "Презентация уже изменилась в другой вкладке. Обновите страницу перед сохранением.",
      409,
    );
  }
    ...
    .set({
      name:
        patch.name === undefined
          ? current.name
          : cleanText(patch.name, "Название презентации", 120) || current.name,
      description:
        patch.description === undefined
          ? current.description
          : cleanText(patch.description, "Описание презентации", 500),
      themeId: selectedTheme,
      accentColor: color(
        patch.accentColor,
        "Акцент",
        patch.themeId === undefined ? current.accentColor : palette.accentColor,
      ),
      backgroundColor: color(
      ...
      textColor: color(
        patch.textColor,
        "Цвет текста",
        patch.themeId === undefined ? current.textColor : palette.textColor,
      ),
      slides:
        patch.slides === undefined ? current.slides : parseSlides(patch.slides),
      updatedAt: now,
    })
    .where(
      and(
...
export async function deletePresentationProject(
  request: Request,
  idValue: unknown,
