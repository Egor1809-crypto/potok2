...
export function parseEmailReview(value: unknown): AiEmailReview {
  validateEmailSchema(value, aiEmailReviewSchema);
  const row = object(value);
  return { score: number(row.score, 0, 100), issues: list(row.issues, 12).map(value => { const item = object(value); return { severity: choice(item.severity, ["low", "medium", "high"] as const), blockId: string(item.blockId, 160, true) || undefined, message: string(item.message, 800) }; }), suggestions: list(row.suggestions, 8).map(x => string(x, 800)) };
}
...
export const aiEmailReviewSchema = shape({ score: { type: "number", minimum: 0, maximum: 100 }, issues: { type: "array", maxItems: 12, items: shape({ severity: enumeration(["low", "medium", "high"]), blockId: nullable(text(160)), message: text(800) }) }, suggestions: { type: "array", maxItems: 8, items: text(800) } });
