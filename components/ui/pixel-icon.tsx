...
export function createPixelIcon(name: string, viewBox: string, paths: Array<{d: string; fillRule?: "evenodd"; clipRule?: "evenodd"}>): PixelIcon {
  const Icon = forwardRef<SVGSVGElement, PixelIconProps>(function RuneIcon({ size = 32, color = "currentColor", className, strokeWidth: _strokeWidth, absoluteStrokeWidth: _absoluteStrokeWidth, children, ...props }, ref) {
    return <svg ref={ref} xmlns="http://www.w3.org/2000/svg" width={size} height={size} viewBox={viewBox} fill="none" color={color} shapeRendering="crispEdges" focusable="false" className={["shrink-0", className].filter(Boolean).join(" ")} aria-hidden={props["aria-label"] || props["aria-labelledby"] || props.role === "img" ? undefined : true} data-rune-icon={name} {...props}>{paths.map((path,index)=><path key={index} {...path} fill="currentColor" />)}{children}</svg>;
  });
