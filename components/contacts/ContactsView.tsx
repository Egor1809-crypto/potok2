  ...
  return (
    <div className="space-y-6">
      ...
      <section className="card p-3" aria-label="Статус отправки контактам">
        ...
        <p className="mt-2 text-[10px] text-[var(--text-muted)]">После принятия письма провайдером контакт автоматически переходит в таблицу «Отправлено».
        </p>
      </section>
      ...
      <section className="card p-4" aria-labelledby="base-responsible-title">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-[10px] text-[var(--text-muted)]">Выбрана {sheet === "all" ? "вся база" : `база «${sheet.replace(/^Импорт: /, "")}»`}. Изменение применится ко всем её контактам.</p>
          </div>
          <div className="flex flex-wrap gap-2">
            ...
            <button type="button" onClick={() => void assignResponsibleToBase(false)} disabled={busy || !responsibleId} className="btn btn-primary">Назначить всей базе</button>
            <button type="button" onClick={() => void assignResponsibleToBase(true)} disabled={busy} className="btn btn-secondary">Снять со всей базы</button>
          </div>
        </div>
      </section>
          ...
          <div className="overflow-x-auto">
            <table className="data-table min-w-[980px]">
              ...
              <tbody>{visible.map((contact) => {
                ...
                const latestDelivery = deliveryHistory[contact.id]?.[0];
                return <tr key={contact.id} data-selected={checked} style={{ boxShadow: `inset 4px 0 0 ${responsible?.color ?? creator?.color ?? "#CBD5E1"}` }}>
                  ...
                  <td>{contact.lastContactedAt ? <span><span className="badge badge-success">✓ Отправлено</span>{latestDelivery ? <span title={latestDelivery.statusMessage} className="mt-1 block max-w-48 truncate text-[9px] text-[var(--text-muted)]">{latestDelivery.campaignName} · {latestDelivery.providerId} · {latestDelivery.status}</span> : null}</span> : telegramHref ? <span className="flex flex-wrap gap-1"><a href={telegramHref} target="_blank" rel="noreferrer" className="badge badge-primary">Открыть TG</a><button type="button" disabled={busy} onClick={() => void markContacted(contact)} className="badge badge-neutral">Отметить</button></span> : <span className="badge badge-neutral">Не отправляли</span>}</td>
