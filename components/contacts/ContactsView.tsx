...
function ContactFormDialog({ contact, busy, onClose, onSave }: { contact: ContactRecord | null; busy: boolean; onClose: () => void; onSave: (draft: ContactDraft) => Promise<void> }) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (!draft.email.trim() && !draft.telegramChatId?.trim() && !draft.vkUserId?.trim()) {
      setError("Укажите хотя бы один канал: email, Telegram chat_id или ID ВКонтакте.");
      return;
    }
    try {
      await onSave(draft);
        ...
        <form onSubmit={submit}>
          ...
          <div className="space-y-6 p-5 sm:p-6">
            ...
            <FormGroup title="Основные данные">
              ...
              <div className="grid gap-4 sm:grid-cols-2"><Field label="Email" type="email" value={draft.email} onChange={(value) => update("email", value)} placeholder="Можно оставить пустым для TG/ВК" /><Field label="Телефон" value={draft.phone ?? ""} onChange={(value) => update("phone", value)} /></div>
