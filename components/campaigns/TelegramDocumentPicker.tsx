...
export function TelegramDocumentPicker({ value, filename, onChange }: {
  ...
  return <div className="space-y-2.5">
    ...
    {value ? <div className="flex items-center gap-3 rounded-xl border border-info/25 bg-info-subtle p-3"><FileText aria-hidden="true" className="size-7 shrink-0 text-info" /><div className="min-w-0 flex-1"><p className="truncate text-[12px] font-semibold text-text-strong">{filename || "document.pdf"}</p><p className="mt-0.5 text-[10px] text-text-muted">Документ с текстом сообщения в подписи</p></div><button type="button" onClick={() => onChange(null, null)} className="grid size-8 place-items-center rounded-lg hover:bg-white" aria-label="Убрать PDF"><X aria-hidden="true" className="size-6" /></button></div> : <button type="button" disabled={uploading} onClick={() => inputRef.current?.click()} className="btn btn-secondary w-full gap-2">{uploading ? <LoaderCircle aria-hidden="true" className="size-6 animate-spin" /> : <Upload aria-hidden="true" className="size-6" />}{uploading ? "Загружаем PDF…" : "Добавить PDF до 20 МБ"}</button>}
