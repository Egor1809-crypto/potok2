...
export function EmailExportMenu({ document, name }: { document: BuilderDocument; name: string }) {
  ...
  return <>
    ...
    <Modal open={open} onOpenChange={setOpen} title="Экспорт письма" description={name} size="full">
      <div className="grid min-w-0 gap-5 lg:grid-cols-[minmax(0,1fr)_280px]">
        <div className="min-w-0"><p className="mb-2 text-sm font-medium [overflow-wrap:anywhere]">Тема: {document.subject || "Без темы"}</p><LetterPreview document={document} title="Письмо для экспорта" className="h-[60vh] min-h-64" /></div>
