...
export function AiEmailWizard({ document, onApply }: { document: BuilderDocument; onApply: (next: BuilderDocument) => void }) {
  ...
  return <section className="min-h-0 flex-1 overflow-y-auto bg-surface-subtle/40 px-4 py-8 sm:px-8" aria-labelledby="ai-email-title">
    <form className="mx-auto grid w-full max-w-3xl gap-6" onSubmit={event => { event.preventDefault(); void submit(); }}>
      ...
      <div className="rounded-2xl border border-border bg-surface p-5"><div className="flex flex-wrap items-center gap-3"><Select aria-label="Тип изображения" value={kind} onChange={e => setKind(e.target.value as "photo" | "logo")} options={choices([["photo", "Фото, продукт или баннер"], ["logo", "Логотип"]])} className="max-w-56" /><Button type="button" variant="secondary" disabled={uploading || ai.busy} onClick={() => files.current?.click()}><Paperclip className="size-6" aria-hidden="true" />{uploading ? "Загружаем…" : "Прикрепить файл"}</Button><input ref={files} type="file" accept="image/png,image/jpeg,image/gif,image/webp,text/plain,.txt" hidden onChange={e => { const file = e.target.files?.[0]; if (file) void upload(file); }} /></div><p className="mb-0 mt-3 text-xs leading-5 text-text-muted">Изображения до 4 МБ или текстовый материал TXT. Ваши файлы используются вместо сгенерированной замены.</p>{brief.assets.length ? <ul className="mb-0 mt-3 grid gap-2">{brief.assets.map(asset => <li key={asset.id} className="flex min-w-0 items-center justify-between gap-3 text-sm"><span className="truncate">{asset.filename}</span><Button type="button" size="sm" variant="ghost" disabled={ai.busy} aria-label={`Убрать ${asset.filename}`} onClick={() => update("assets", brief.assets.filter(a => a.id !== asset.id))}><X className="size-6" aria-hidden="true" /></Button></li>)}</ul> : null}</div>
      ...
      <div className="flex flex-wrap items-center gap-3 pb-4"><Button type="submit" variant="primary" disabled={ai.busy || uploading}>{ai.busy ? <LoaderCircle className="size-6 motion-safe:animate-spin" aria-hidden="true" /> : <ArrowRight className="size-6" aria-hidden="true" />}Создать письмо</Button>{ai.busy ? <Button type="button" variant="secondary" onClick={ai.cancel}>Остановить</Button> : <span className="text-xs text-text-muted">Результат откроется в редакторе. Любой блок можно изменить.</span>}</div>
    </form>
  </section>;
}
